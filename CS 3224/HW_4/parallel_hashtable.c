...
pthread_mutex_t *table_locks[NUM_BUCKETS];
...
bucket_entry * retrieve(int key) {
    // Assuming that the wording of the assignment intends to imply that split-second differences on reading aren't critical and therefore there is no lock needed on read
    bucket_entry *b;
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < NUM_BUCKETS; i++) {
        table_locks[i] = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t));
